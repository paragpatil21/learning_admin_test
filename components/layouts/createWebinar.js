...
export default function AddLocationDialog(props) {
  ...
  const [courses, setCourses] = useState(["WAD","ITES","ALL"]);
  ...
  function submitLocation(e) {
    ...
    // } else {
    axios
      .post(API_URL + "webinar/add_webinar.php", formData, config)
      .then(function (response) {
...
<div className="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-2">
                        ...
                        <div className="sm:col-span-2">
                          <select size="7" multiple onChange={(e) =>{ value1= Array.from(e.target.selectedOptions,option =>option.value),setCourse(value1.join(','))}} id="course" name="course" className="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                            <option>Select Course</option>
                            
                            
                            {courses ? courses.map((e) => <option value={e}>{e}</option>) : ""}
                          </select>
                        </div>
                      </div>
                      {course}
