...
import Chapters from "../../components/layouts/chaptersList";
import AssessmentChapters from "../../components/layouts/assessmentChapters";
const Enroll = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.open && !assessmentData && !coursesData && props.id) {
      axios
        .post(API_URL + "students/get_module_courses.php", {
          level: 1,
          student: props.id,
        })
        .then(function (response) {
          setCoursesData(response.data?.data);
          setAssessmentData(response.data?.assessmentData);
        });
    }
    // if (props.open && !userdata1) {
    //   axios
    //     .post(API_URL + "students/get_module_assessments.php", {
    //       level: 1,
    //       student: props.id,
    //     })
    //     .then(function (response) {
  ...
  return (
    <Transition.Root show={isShowing} as={Fragment}>
      <Dialog as="div" static className="bg-gray-800 bg-opacity-50 fixed inset-0 overflow-hidden z-40" open={isShowing} onClose={closed}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="px-4 py-6 bg-gray-50 sm:px-6">
                      <div className="flex items-start justify-between space-x-3">
                        <div className="space-y-1">
                          <Dialog.Title className="text-lg font-medium text-gray-900">Student Progress</Dialog.Title>
                          <p className="text-xs text-gray-500">Bellow is the complete student progress of courses and assessments.</p>
                        </div>
                          ...
                          {tabs.map((tab, tabIdx) => (
                            <a
                              ...
                              className={classNames(
                                ...
                                "group relative min-w-0 flex-1 overflow-hidden bg-white py-2 md:py-4 px-4 text-sm font-medium text-center hover:bg-gray-50 focus:z-10"
                              )}
                              aria-current={tab.current ? "page" : undefined}>
                              <span>{tab.name}</span>
                              <span aria-hidden="true" className={classNames(tab.current ? "bg-indigo-500" : "bg-transparent", "absolute inset-x-0 bottom-0 h-0.5")} />
                            </a>
                          ))}
                        </nav>
                      </div>
                    ...
                    <div class=" bg-gray-200">
                      {coursesdisplay === true && assessmentdisplay === false ? (
                        <div class="border-0 md:border border-gray-200 rounded bg-white divide-gray-200 overflow-hidden">
                          {coursesData
                            ? coursesData.map((data) => (
                                <div key={data?.ID} class="m-0 md:m-5 border md:border-2 rounded-0 md:rounded-xl items-center text-gray-700 hover:text-gray-700 hover:bg-gray-50 active:bg-white" href="javascript:void(0)">
                                  <div>
                                    <p class="p-3 md:p-4 font-semibold text-xs md:text-sm">{data?.ModuleTitle}</p>
                                  </div>
                                  <div class="auto-rows-max grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 sm:px-8 xl:px-0 overflow-x-auto flex-nowrap">
                                    <Chapters courses={data.Courses} module_id={data.ID} title={data.ModuleTitle} student={props.id} />
                                  </div>
                                </div>
                              ))
                            : ""}
                        ...
                        <div class="border-0 md:border border-gray-200 rounded bg-white divide-gray-200 overflow-hidden">
                          {assessmentData
                            ? assessmentData.map((data) => (
                                <div key={data?.ID} class="m-0 md:m-5 border md:border-2 rounded-0 md:rounded-xl items-center text-gray-700 hover:text-gray-700 hover:bg-gray-50 active:bg-white" href="javascript:void(0)">
                                  <div>
                                    <p class="p-3 md:p-4 font-semibold text-xs md:text-sm flex justify-between">
                                      {data?.ModuleTitle}
                                      <span>{data?.quizStatus}</span>
                                    </p>
                                  </div>
                                  <div class="auto-rows-max grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 sm:px-8 xl:px-0 overflow-x-auto flex-nowrap">
                                    <AssessmentChapters courses={data.Courses} module_id={data.ID} title={data.ModuleTitle} student={props.id} />
                                  </div>
                                </div>
                              ))
                            : ""}
