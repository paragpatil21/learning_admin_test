...
import AddAssetDialog from "../../components/layouts/createQuiz1";
import EditAssetDialog from "../../components/layouts/editQuiz1";
...
export default function Assets() {
  ...
  useEffect(() => {
    axios
      .post(API_URL + "quiz1/get_quiz1.php", {
  ...
  function deleteAssetFunc(id) {
    ...
    axios
      .post(API_URL + "quiz1/delete_quiz1.php", {
        id: id,
      })
      .then(function (response) {
  ...
  return (
    <Template page="Module Assessments">
      <div className=" overflow-auto">
                  ...
                  <div className="max-w-3xl mx-auto mb-5 px-0 sm:px-0 md:flex md:items-center md:justify-between md:space-x-5 lg:mb-0 lg:max-w-7xl lg:px-0">
                    <div className="mt-6 flex flex-col-reverse justify-stretch space-y-4 space-y-reverse sm:flex-row-reverse sm:justify-start sm:space-x-reverse sm:space-y-0 sm:space-x-3 md:mt-0 md:flex-row md:space-x-3">
                      ...
                      <select onChange={(e) => setCourseFilter(e.target.value)} value={coursefilter} id="role" name="role" className="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                        ...
                        {users2 ? users2.map((data) => <option value={data.ModuleTitle}>{data.ModuleTitle}</option>) : ""}
                      </select>
                          ...
                          <thead className="bg-gray-50">
                            <tr>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <span class="md:table-cell hidden">Question</span>{" "}
                              </th>
                            ...
                            {users1 ? (
                              users1.map((person) => (
                                <tr key={person?.ID}>
                                      ...
                                      <div className="ml-4 space-y-2 md:space-y-0">
                                        <div className="flex md:items-center mr-2 md:mr-0 md:justify-between">
                                          <div className="inline-block ml-3 md:ml-0">
                                            <span class="text-sm md:hidden font-medium">Question :</span>
                                            <div className="text-sm  text-gray-900 inline-block ml-1 md:ml-0">{person?.Question}</div>
                                          </div>
                                        </div>
                                        ...
                                        <div className="flex  md:hidden ml-7 flex-1">
                                          <span class="text-sm md:hidden font-medium">Module:</span>
                                          <div className="text-sm  text-gray-900 md:hidden ml-1 ">{person?.ModuleTitle}</div>
                                        </div>
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">{person?.ModuleTitle}</td>
        ...
        <AddAssetDialog locations={filterLocations} statuses={filterStatuses} assetTypes={filterAssetTypes} open={addAsset} onChangeOpen={() => setAddAsset(false)} assetAdded={(e) => setAssetsUpdated(e)} successMessage={(e) => notifyOpen(e)} />
        <EditAssetDialog
          id={editAssetId}
