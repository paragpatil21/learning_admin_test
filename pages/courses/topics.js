...
import { ArrowNarrowLeftIcon, CheckIcon, HomeIcon, PaperClipIcon, QuestionMarkCircleIcon, SearchIcon, ThumbUpIcon, UserIcon, PencilAltIcon } from "@heroicons/react/solid";
import { BellIcon, EyeIcon, MenuIcon, UploadIcon, XIcon, PlusIcon, TrashIcon } from "@heroicons/react/outline";
import Head from "next/head";

import AddTopicDialog from "../../components/layouts/createTopic";
import EditTopicDialog from "../../components/layouts/editTopic";
...
export default function Assets() {
  ...
  useEffect(() => {
    axios
      .post(API_URL + "courses/get_topics.php", {
      ...
      .then(function (response) {
        ...
        setUsers2(response.data?.courses);
  ...
  }, [searchAssetKeyword, mysearch, coursefilter, myfilter, assetsUpdated, filterState, filterLocation, filterAssetType, filterStatus, pagination]);
  ...
  function deleteAssetFunc(id) {
    ...
    axios
      .post(API_URL + "courses/delete_topic.php", {
        id: id,
      })
      .then(function (response) {
            ...
            <main className="py-10">
              <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
                <div className="flex flex-col justify-center lg:flex-row lg:justify-between lg:space-x-5">
                  ...
                  {addedSuccess ? <Notification show={showNotification} type="success" message={addedSuccess} icon="success" /> : ""}
                  <div className="max-w-3xl mx-auto mb-5 px-0 sm:px-0 md:flex md:items-center md:justify-between md:space-x-5 lg:mb-0 lg:max-w-7xl lg:px-0">
                    <div className="mt-6 flex flex-col-reverse justify-stretch space-y-4 space-y-reverse sm:flex-row-reverse sm:justify-start sm:space-x-reverse sm:space-y-0 sm:space-x-3 md:mt-0 md:flex-row md:space-x-3">
                      <select onChange={(e) => setMyFilter(e.target.value)} value={myfilter} id="role" name="role" className="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                      ...
                      <select onChange={(e) => setCourseFilter(e.target.value)} value={coursefilter} id="role" name="role" className="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                        <option>Select Course</option>

                        {users2 ? users2.map((data) => <option value={data.Title}>{data.Title}</option>) : ""}
                      </select>
                      ...
                      {(userRole=="contentwriter" || userRole=="superadmin")?(
                ...
                <div id="update_buttons" className={updateButtons ? "" : "hidden"}>
                  <div className="max-w-3xl mx-auto mb-5 px-0 sm:px-0 md:flex md:items-center md:justify-between md:space-x-5 lg:max-w-7xl lg:px-0">
                    <div className="mt-6 flex flex-col-reverse justify-stretch space-y-4 space-y-reverse sm:flex-row-reverse sm:justify-start sm:space-x-reverse sm:space-y-0 sm:space-x-3 md:mt-0 md:flex-row md:space-x-3">
                      ...
                      <div className="shadow border-b border-gray-200 sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                              {/* <th scope="col" className=" hidden md:table-cell pl-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <input onChange={(e) => checkboxAllTrigger(e)} id="select_all" name="select_all" type="checkbox" className="  h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                              </th> */}
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {/* <span class="md:hidden ml-4 ">
                                  {" "}
                                  <input onChange={(e) => checkboxAllTrigger(e)} id="select_all" name="select_all" type="checkbox" className="mr-2 h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                                </span> */}
                                <span class="md:table-cell hidden">Title</span>{" "}
                              </th>
...
{(userRole=="contentwriter" || userRole=="superadmin")?(
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {users1 ? (
                              users1.map((person) => (
                                <tr key={person?.ID}>
                                  {/* <td className="pl-6 py-4 whitespace-nowrap hidden md:table-cell">
                                    <div className="flex items-center">
                                      <input onChange={(e) => checkboxTrigger(e)} name="list_selector" value={person?.AssetId} type="checkbox" className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                                    </div>
                                  </td> */}
                                    ...
                                    <div className="flex ">
                                      <div className="ml-4 space-y-2 md:space-y-0">
                                        <div className="flex md:items-center mr-2 md:mr-0 md:justify-between">
                                          {/* <input onChange={(e) => checkboxTrigger(e)} name="list_selector" value={person?.AssetId} type="checkbox" className="mt-1  md:mt-0 h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded md:hidden" /> */}

                                          <div className="inline-block ml-3 md:ml-0">
                                            <span class="text-sm md:hidden font-medium">Title :</span>
                                            <div className="text-sm  text-gray-900 inline-block ml-1 md:ml-0">{person?.Title}</div>
                                          </div>
                                        </div>

                                        {/* <div className="flex  md:hidden ml-7 flex-1">
                                          <span class="text-sm md:hidden font-medium">Answer :</span>
                                          <div className="text-sm    md:hidden ml-1">{person["Option" + person?.Answer]}</div>
                                        </div> */}
                                        <div className="flex  md:hidden ml-7 flex-1">
                                          <span class="text-sm md:hidden font-medium">Chapter Title:</span>
                                          <div className="text-sm  text-gray-900 md:hidden ml-1 ">{person?.CourseTitle}</div>
                                        </div>
                                        ...
                                        {(userRole=="contentwriter" || userRole=="superadmin")?(
                                          ...
                                          <a onClick={() => handleEditAsset(person?.ID)} className=" md:hidden text-indigo-600 hover:text-indigo-900 cursor-pointer p-2">
                                            <PencilAltIcon className="w-4 h-4 inline-block" />
                                          </a>{" "}
                                          <a onClick={() => handleDeleteAsset(person?.ID)} className=" md:hidden  text-red-600 hover:text-red-900 cursor-pointer p-2">
                                  ...
                                  {/* <td className="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                    <div className="text-sm text-gray-900">{person["Option" + person?.Answer]}</div>
                                     <div className="text-sm text-gray-500">City: {person?.email}</div> 
                                  </td> */}
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">{person?.CourseTitle}</td>
                                  ...
                                  {(userRole=="contentwriter" || userRole=="superadmin")?(
                                    ...
                                    <a onClick={() => handleEditAsset(person?.ID)} className="text-indigo-600 hover:text-indigo-900 cursor-pointer p-2">
                                      <PencilAltIcon className="w-4 h-4 inline-block" />
                                    </a>
                                    |{" "}
                                    <a onClick={() => handleDeleteAsset(person?.ID)} className="text-red-600 hover:text-red-900 cursor-pointer p-2">
                                      <TrashIcon className="w-4 h-4 inline-block" />
                                    </a>
                                    </>
                                                          ):("")}
        ...
        <ConfirmDialog id={deleteAssetId} message="You want to delete the asset" open={deleteAsset} onClosed={() => setDeleteAsset(false)} onConfirmed={() => deleteAssetFunc(deleteAssetId)} />

        <AddTopicDialog locations={filterLocations} statuses={filterStatuses} assetTypes={filterAssetTypes} open={addAsset} onChangeOpen={() => setAddAsset(false)} assetAdded={(e) => setAssetsUpdated(e)} successMessage={(e) => notifyOpen(e)} />
        <EditTopicDialog
          id={editAssetId}
